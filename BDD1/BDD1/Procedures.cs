...
namespace BDD1
{
    public class Procedures
    {
        ...
        public static int estudiante_borrar(int id)
        {
            ...
            cmd.Parameters.Add("@id", SqlDbType.Int).Value = id;
            Post post = new Post();
            cmd.Parameters.Add("@PostIn", SqlDbType.VarChar).Value = post.PostIn;
            cmd.Parameters.Add("@PostBy", SqlDbType.VarChar).Value = post.PostBy.ToString();
            cmd.Parameters.Add("@PostDate", SqlDbType.Date).Value = post.PostDate;
            cmd.Parameters.Add("@result", SqlDbType.Int).Direction = ParameterDirection.ReturnValue;
            con.Open();
            cmd.ExecuteNonQuery();
            int ret = int.Parse(cmd.Parameters["@result"].Value.ToString());
